...
Bezier.prototype.split = function ()
{
	...
	var left = new Array(this.order),
		right = new Array(this.order);
...
Bezier.prototype.measureLength = function (tolerance)
{
	...
	do
	{
		...
		if (polylen - chordlen <= tolerance) sum += polylen;
		else queue = queue.concat(b.split());
	} while (queue.length);
	return (this.measureLength = function ()
	{
		return sum
...
Bezier.prototype.draw = function (ctx)
{
	...
	else error("don't know how to draw an order *" + this.order + " bezier");
};
